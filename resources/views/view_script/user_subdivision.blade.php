...
//Editing User in Subdivision 
@if(isset($userdesig))
$('#editusersubdivision').on('submit',function(e){
        $.ajaxSetup({
        header:$('meta[name="_token"]').attr('content')
    });
    var formData = $(this).serialize();
        $.ajax({
            type:"PUT",
            url: "{{url('/usersubdivision/') }}"+'/'+ {{ $userdesig ->iddesgignationdistrictmapping }},
            data:formData,
            dataType: 'json',
            success:function(data){
                if( data[Object.keys(data)[0]] === 'SUCCESS' ){		//True Case i.e. passed validation
                window.location = "{{url('usersubdivision')}}" +'/'+ {{ $userdesig ->idUser }} + '/edituser';
                }
                else {					//False Case: With error msg
                $("#msg").html(data);	//$msg is the id of empty msg
                }
