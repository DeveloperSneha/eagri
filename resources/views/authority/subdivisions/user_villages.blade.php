...
@section('content')
<a href="{{url('authority/subdivisions/addviuser/create')}}" class="btn btn-success" style="margin-bottom: 20px;">Add Existing</a>
<!-------------------New User---------------------------------------------------------------------->
<div id="formerrors"></div>
<div class="panel panel-default">
    <div class="panel-heading"><strong>ADD  User In Village</strong></div>
    {!! Form::open(['url' => 'authority/subdivisions/addviuser','class'=>'form-horizontal','id'=>'uservillage']) !!}
        ...
        <table class="table table-bordered table-hover table-striped dataTable" id='table1'>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $i }}</td>
                    <td>{{ $var->userName}}</td>
                    <td> <a href='{{url('/authority/subdivisions/addviuser/'.$var->idUser.'/details')}}' class="btn btn-xs btn-warning">Edit</a>
                    </td>
                </tr>
                <?php $i++; ?>
                @endforeach
...
<script>
    ...
    var formData = $(this).serialize();
        $.ajax({
            type:"POST",
            url: "{{url('/authority/subdivisions/addviuser/') }}",
            data:formData,
            dataType: 'json',
            success:function(data){
                if( data[Object.keys(data)[0]] === 'SUCCESS' ){		//True Case i.e. passed validation
                window.location = "{{url('authority/subdivisions/addviuser')}}";
                }
