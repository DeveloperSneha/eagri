...
<div class="panel panel-default tab-pane fade in active" id='new'>
    ...
    <div class="panel-body">
        {!! Form::model( $userdesig, ['route' => ['blockuseradd.update', $userdesig->iddesgignationdistrictmapping], 'method' => 'patch','class'=>'form-horizontal','id'=>'blockuseradd'] ) !!}
...
@if(isset($userdesig))
$('#blockuseradd').on('submit',function(e){
        $.ajaxSetup({
    ...
    var formData = $(this).serialize();
        $.ajax({
            type:"PUT",
            url: "{{url('/authority/subdivisions/blockuseradd/') }}"+'/'+ {{ $userdesig ->iddesgignationdistrictmapping }},
            data:formData,
            dataType: 'json',
            success:function(data){
                if( data[Object.keys(data)[0]] === 'SUCCESS' ){     //True Case i.e. passed validation
                window.location = "{{url('/authority/subdivisions/blockuseradd')}}" +'/'+ {{ $userdesig ->idUser }} + '/details';
                }
