...
<div class="panel panel-default tab-pane fade in active" id='new'>
    ...
    <div class="panel-body">
        {!! Form::model( $userdesig, ['route' => ['addblockuser.update', $userdesig->iddesgignationdistrictmapping], 'method' => 'patch','class'=>'form-horizontal','id'=>'addblockuser'] ) !!}
...
@if(isset($userdesig))
$('#addblockuser').on('submit',function(e){
        $.ajaxSetup({
    ...
    var formData = $(this).serialize();
        $.ajax({
            type:"PUT",
            url: "{{url('/authority/districts/addblockuser/') }}"+'/'+ {{ $userdesig ->iddesgignationdistrictmapping }},
            data:formData,
            dataType: 'json',
            success:function(data){
                if( data[Object.keys(data)[0]] === 'SUCCESS' ){     //True Case i.e. passed validation
                window.location = "{{url('/authority/districts/addblockuser')}}" +'/'+ {{ $userdesig ->idUser }} + '/details';
                }
