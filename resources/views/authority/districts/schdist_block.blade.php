...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        {!! Form::open(['url' => 'authority/districts/schblockdist','class'=>'form-horizontal','id'=>'blockdistribution']) !!}
...
<script>
    ...
    $('select[name="idSchemeActivation"]').on('change', function() {
        ...
        if(schemeActivationID) {
            $.ajax({
                url: "{{url('/authority/districts/schblockdist') }}"+'/' +schemeActivationID + '/subdivisions',
                type: "GET",
                dataType: "json",
                success:function(data) {
                ...
                success:function(data) {
                    $('#idBlock').empty();
                       $.each(data, function(key, value) {
                            ...
                            var checkbox="<tr>\n\
                                                <td><label for="+value+">"+value+"</label><span id='errorblock"+key+"' class='help-block'></span></td>\n\
                                                <td><input type='checkbox' class='count_dist' id='block'  value="+key+" name='blocks["+key+"][idBlock]'><label for="+value+"></label></td>\n\
                                                <td><input type = 'text' name='blocks["+key+"][areaBlock]'  onchange='getArea("+key+")' id='areablock"+key+"' class='form-control' onkeypress='return isNumberKey(event)' minlength='1' maxlength='12'><span id='errorarea"+key+"' class='help-block'></span><input type='hidden' id='hiddenarea"+key+"'></td>\n\
                                                <td><input type = 'text' name='blocks["+key+"][amountBlock]' id='amtblock"+key+"' class='form-control' onkeydown='return false;'><span id='erroramt"+key+"' class='help-block'></span><input type='hidden' id='hiddenamount"+key+"'></td>\n\
                                            </tr>"
                                $("#idBlock").append($(checkbox));
                        });
                }
            });
